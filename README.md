...
>
> **最新版本 (v1.5.3)** — 2026-02-23
>
> | 版本 | 日期 | 核心改动 |
> |------|------|---------|
> | **v1.5.3** | 2026-02-23 | LLM 兜底机制修复：触发率从 0% → 正常，兼容 GLM 思考模型 |
> | **v1.5.2** | 2026-02-23 | TF-IDF 向量检索 + RRF 混合检索，完全离线零 API，检索结果从 8 → 16 条 |
> | **v1.5.1** | 2026-02-23 | Spreading Activation（ACT-R）+ Soul Health 历史趋势记录 |
> | **v1.5.0** | 2026-02-23 | 三维检索评分 + 自动 Reflection + Identity 保护 + Soul Health 监控 |
> | **v1.4.0** | 2026-02-15 | 百万级扩展：分片索引、多级缓存、异步索引器 |
> | **v1.3.0** | 2026-02-10 | OpenClaw Gateway 深度集成 + Phase 1 基础能力补齐 |
> | **v1.2.0** | 2026-02-07 | 稳定性优化：SQLite 后端、QMD 语义索引、冷启动优化 |
> | **v1.1.7** | 2026-02-06 | 智能 LLM 集成：语义复杂度检测，自动决定是否调用 LLM |
> | **v1.1.6** | 2026-02-06 | 引号实体提取 + 去重阈值改用相对比例（Crabby 测试反馈修复） |
> | **v1.1.5** | 2026-02-06 | 三层实体识别：规则 → 词典 → LLM 兜底 |
> | **v1.1.4** | 2026-02-05 | 访问追踪 + 时间敏感检测 + 三级漏斗衰减 |
> | **v1.1.3** | 2026-02-05 | LLM 兜底机制：规则优先 + LLM 判断不确定区间 |
> | **v1.1.2** | 2026-02-05 | 实体识别优化：支持正则模式匹配动态实体 |
> | **v1.1.1** | 2026-02-05 | 分词优化 + 冲突柔性降权（新记忆覆盖旧记忆时自动降权） |
> | **v1.1.0** | 2026-02-05 | 完整 Consolidation 流程（Phase 1-7）+ search 命令 |
> | **v1.0.0** | 2026-02-04 | 三层记忆架构、基础 CLI、衰减机制、实体档案系统 |
>
...
**已完成**
- [x] 接入 OpenClaw session 数据，实现自动记忆采集（collect_from_sessions.py）
- [x] 优化检索算法：TF-IDF 向量检索 + RRF 混合检索 + Spreading Activation
- [x] LLM 兜底机制（GLM-4.5-air，规则优先 + LLM 判断不确定区间）
- [x] Identity 保护：身份类记忆衰减减半，永不轻易遗忘
- [x] Soul Health 监控：S = 0.25·T + 0.35·C + 0.40·I 三维健康评分

**进行中 / 规划中**
- [ ] 语义知识图谱：在现有 relations.json 上加语义边（就读于/从事/属于），实现语义推理检索
- [ ] 可视化面板，让记忆系统"可见"
- [ ] 大规模压测（10000+ 条记忆）
