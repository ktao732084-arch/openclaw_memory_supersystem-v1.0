# Memory System v1.2.4 - SQLite 后端使用指南

## 🎯 概述

v1.2.4 引入了 SQLite 后端，与现有 JSONL 系统并存，支持平滑迁移。

### 核心特性

- ✅ **双后端支持**: JSONL 和 SQLite 可选
- ✅ **平滑迁移**: 不影响现有系统
- ✅ **性能提升**: 访问统计 O(N) → O(1)
- ✅ **关系支持**: 三元组表解决实体冲突
- ✅ **向后兼容**: 保留 JSONL 备份

---

## 📦 文件说明

| 文件 | 说明 |
|------|------|
| `sqlite_backend.py` | SQLite 后端核心实现 |
| `backend_adapter.py` | 双后端适配器 |
| `benchmark.py` | 性能对比测试 |

---

## 🚀 快速开始

### 1. 测试 SQLite 后端

```bash
cd scripts
python3 sqlite_backend.py /root/.openclaw/workspace/memory test
```

### 2. 迁移数据

```bash
# 迁移 JSONL -> SQLite（自动备份）
python3 sqlite_backend.py /root/.openclaw/workspace/memory migrate
```

### 3. 性能测试

```bash
# 对比 JSONL vs SQLite 性能
python3 benchmark.py /root/.openclaw/workspace/memory
```

---

## 🔧 配置后端

### 方法 1: 通过配置文件

编辑 `memory/config.json`:

```json
{
  "storage": {
    "backend": "sqlite"  // 或 "jsonl"
  }
}
```

### 方法 2: 通过命令行

```bash
# 切换到 SQLite
python3 backend_adapter.py /root/.openclaw/workspace/memory config sqlite

# 切换回 JSONL
python3 backend_adapter.py /root/.openclaw/workspace/memory config jsonl
```

---

## 📊 性能对比

基于 100 条记忆的测试结果：

| 操作 | JSONL | SQLite | 提升 |
|------|-------|--------|------|
| 访问统计更新 | ~50ms (O(N)) | 5.36ms (O(1)) | **9.3x** |
| 实体搜索 | ~20ms | 1.29ms | **15.5x** |
| 全量读取 | 1.04ms | 2.73ms | 0.4x |

**结论**: 
- ✅ 写操作（访问统计）大幅提升
- ✅ 查询操作（实体搜索）大幅提升
- ⚠️ 小数据量全量读取 JSONL 更快（但差异可忽略）

---

## 🗄️ 数据库结构

### 主表: memories

```sql
CREATE TABLE memories (
    id TEXT PRIMARY KEY,
    type TEXT CHECK(type IN ('fact', 'belief', 'summary')),
    content TEXT NOT NULL,
    importance REAL,
    score REAL,
    access_boost REAL,
    final_score REAL GENERATED ALWAYS AS (score + access_boost) STORED,
    created TEXT,
    last_accessed TEXT,
    access_count INTEGER,
    retrieval_count INTEGER,
    state INTEGER,  -- 0:Active, 1:Archived, 2:Junk
    ...
);
```

### 实体表: memory_entities

```sql
CREATE TABLE memory_entities (
    memory_id TEXT,
    entity TEXT,
    PRIMARY KEY (memory_id, entity)
);
```

### 关系表: memory_relations

```sql
CREATE TABLE memory_relations (
    memory_id TEXT,
    subject TEXT,
    relation_type TEXT,
    object TEXT,
    superseded INTEGER,  -- 是否被取代
    ...
);
```

---

## 🔄 迁移流程

### 自动迁移

```bash
python3 sqlite_backend.py /root/.openclaw/workspace/memory migrate
```

**流程**:
1. 自动备份 JSONL 文件（.backup 后缀）
2. 创建 SQLite 数据库
3. 逐条迁移记忆
4. 验证数据完整性

### 手动回滚

如果需要回滚到 JSONL：

```bash
# 1. 恢复备份
cd /root/.openclaw/workspace/memory/layer2/active
cp facts.jsonl.backup facts.jsonl
cp beliefs.jsonl.backup beliefs.jsonl
cp summaries.jsonl.backup summaries.jsonl

# 2. 删除 SQLite 数据库
rm ../memories.db*

# 3. 切换配置
python3 backend_adapter.py /root/.openclaw/workspace/memory config jsonl
```

---

## 🧪 测试验证

### 1. 数据完整性测试

```python
from sqlite_backend import SQLiteBackend
from pathlib import Path

backend = SQLiteBackend(Path('/root/.openclaw/workspace/memory'))

# 检查统计
stats = backend.get_stats()
print(f"总记忆数: {stats['total']}")
print(f"Facts: {stats['facts']}")
print(f"Beliefs: {stats['beliefs']}")
print(f"Summaries: {stats['summaries']}")
```

### 2. 搜索功能测试

```python
# 实体搜索
results = backend.search_by_entities(['[用户]'], limit=5)
for r in results:
    print(f"[{r['final_score']:.2f}] {r['content'][:50]}...")
```

### 3. 访问统计测试

```python
# 更新访问统计
backend.update_access_stats('f_20260207_a6b928', 'retrieval')

# 验证更新
memory = backend.get_memory('f_20260207_a6b928')
print(f"访问次数: {memory['access_count']}")
```

---

## 🚨 注意事项

### 1. 数据安全

- ✅ 迁移前自动备份 JSONL
- ✅ SQLite 使用 WAL 模式（防止数据损坏）
- ✅ 支持随时回滚到 JSONL

### 2. 性能考虑

- ✅ 小数据量（<1000条）: JSONL 和 SQLite 性能相近
- ✅ 大数据量（>1000条）: SQLite 性能优势明显
- ✅ 写密集场景: SQLite 大幅领先

### 3. 兼容性

- ✅ 不影响现有 JSONL 系统
- ✅ 可以同时保留两套数据
- ✅ 支持双写（JSONL + SQLite）

---

## 📋 下一步计划

### v1.2.5: 关系提取

- [ ] 实现关系提取逻辑
- [ ] 集成到 Phase 3
- [ ] 基于关系的冲突解决

### v1.3.0: 完全切换

- [ ] 默认使用 SQLite
- [ ] 移除 JSONL 依赖
- [ ] 性能优化

---

## 🐛 故障排查

### 问题 1: 迁移失败

**症状**: `CHECK constraint failed: type IN ('fact', 'belief', 'summary')`

**原因**: JSONL 中缺少 type 字段

**解决**: 已修复，从文件名推断类型

### 问题 2: SQLite 不可用

**症状**: `⚠️ SQLite 后端不可用`

**原因**: `sqlite_backend.py` 未找到

**解决**: 确保在 `scripts/` 目录下运行

### 问题 3: 数据不一致

**症状**: SQLite 和 JSONL 记忆数不同

**原因**: 可能有重复 ID 或格式错误

**解决**: 重新迁移，检查 JSONL 格式

---

## 📞 支持

如有问题，请查看：
- [CHANGELOG.md](../CHANGELOG.md)
- [v1.2.4-sqlite-migration-final.md](v1.2.4-sqlite-migration-final.md)

---

**文档版本**: v1.0  
**创建时间**: 2026-02-14  
**作者**: [助手]
