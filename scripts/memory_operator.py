...
class MemoryOperator:
    ...
    def __init__(self, llm_client=None, similarity_threshold: float = 0.7, backend=None):
        ...
        # 统计信息
        self.stats = {"total": 0, "add": 0, "update": 0, "delete": 0, "noop": 0, "llm_calls": 0}

    def decide_operation(self, new_memory: Dict, existing_memories: List[Dict]) -> Tuple[str, Optional[Dict]]:
        """
        决定对新记忆执行什么操作
    ...
    def _find_conflicts_by_similarity(self, new_memory: Dict, existing_memories: List[Dict]) -> List[Dict]:
        """
        通过语义相似度查找潜在冲突
        ...
        for old in existing_memories:
            ...
            # 2. 计算语义相似度
            similarity = self._calculate_similarity(new_memory["content"], old["content"])
    ...
    def _tokenize(self, text: str) -> List[str]:
        ...
        # 过滤停用词
        stopwords = {"的", "了", "在", "是", "我", "你", "他", "她", "它", "the", "a", "an", "is", "are", "was", "were"}
    ...
    def _llm_decide(self, new_memory: Dict, conflicts: List[Dict]) -> Tuple[str, Optional[Dict]]:
        """
        使用 LLM 决策（复杂场景）
    ...
    def _rule_based_decide(self, new_memory: Dict, conflicts: List[Dict]) -> Tuple[str, Optional[Dict]]:
        """
        基于规则的决策（简单场景）
...
if __name__ == "__main__":
    ...
    # 测试 1: 明确的噪声
    print("\n📝 测试 1: 明确的噪声")
    noise_memory = {"content": "3 + 5 等于多少", "entities": []}
