...
返回 JSON：{"importance": 0.8, "category": "preference_relation", "reason": "简短理由"}"""
    ...
    fallback_result = json.dumps(
        {"importance": rule_importance, "category": rule_category, "reason": "LLM fallback to rule"}
    )
...
class LLMIntegrationStats:
    ...
    def print_summary(self):
        print("\n📊 LLM 集成统计:")
        print(
            f"   Phase 2 (筛选): {self.phase2_calls} 调用, {self.phase2_successes} 成功, {self.phase2_fallbacks} 回退"
        )
        print(
            f"   Phase 3 (提取): {self.phase3_calls} 调用, {self.phase3_successes} 成功, {self.phase3_fallbacks} 回退"
        )
