...
def get_laike_data(token, target_phones):
    """快速获取来客数据，只处理目标手机号"""
    ...
    while True:
        ...
        if result.get('code') == 0:
            ...
            count += len(records)
            print(f"   已扫描 {count} 条...", end='\r', flush=True)
    ...
    print(f"   ✓ 扫描完成，共 {count} 条")
    return arrival_customers
...
def main():
    ...
    # 2. 加载来客数据并匹配
    print("\n[2/4] 扫描来客数据并匹配...")
    arrival_customers = get_laike_data(token, shunding_phones)
