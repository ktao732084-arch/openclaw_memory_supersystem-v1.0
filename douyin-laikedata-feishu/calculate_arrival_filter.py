...
def main():
    ...
    print(f"   手机号列表: {sorted(shunding_phones)}")
    
    # 2. 逐个搜索来客数据
    print("\n[2/4] 搜索来客数据...")
    arrival_customers = []
    
    for i, phone in enumerate(sorted(shunding_phones), 1):
        print(f"   搜索 {i}/{len(shunding_phones)}: {phone}...", end=' ', flush=True)
